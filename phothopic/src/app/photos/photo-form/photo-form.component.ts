...
export class PhotoFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.photoForm = this.formBuilder.group({
      ...
      description: ['', [Validators.minLength(5), Validators.maxLength(100), Validators.required]],
      allowComments: [true]
    });
  }
    ...
    this.photoService.upload(description, allowComments, this.file)
      ...
      .subscribe(
        (event: HttpEvent<any>) => {
          ...
          } else if(event instanceof HttpResponse) { // serve event.type == HttpEventType.Response também 
            this.alertService.success('Upload complete', true);
          }
        }, 
        err => this.alertService.danger('Upload failed', true));
  }
